...
`plume-move-opening-lab` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies opening behavior through node-edge fixtures, with cycle and reachability reports and synthetic fixtures only.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 2.
